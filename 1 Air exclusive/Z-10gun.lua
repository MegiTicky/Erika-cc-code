...
local cannon1 = peripheral.find("cbcmf_compact_cannon_mount")
if not(cannon1) then
    error("cannon1 not found")
end

local cannonSourceChannel = 701
local controlChannel = 1100
...
local function readConfig(filename)
    if not fs.exists(filename) then
        ...
        controlChannel = askUser("Input the control channel", "1100")
...
local function aimCannon(targetPos, targetVel, sourceX, sourceY, sourceZ)
    if sourceX and sourceY and sourceZ then
        ...
        if bal then
            ...
            preCompensatedPitchTable = bal.calculatePitch(sourceTemp, targetTemp, projectileSpeed, 12, -60, 60, 0.05, 0.99, 1, 1000000, 5, 20, false)
        end
        ...
        if math.abs(math.abs(requiredRelativePitch) - math.abs(cannon1.getPitch())) > 0.2 then
            cannon1.setPitch(requiredRelativePitch+90)
        end
        if math.abs(requiredRelativeYaw - cannon1.getYaw()) > 0.1 then
            cannon1.setYaw(requiredRelativeYaw-180)
        end
    end
end
            ...
            if pInfo.nickname then
                ...
                if lockedTarget then
                    ...
                    if lockedTarget.pos then
                        aimCannon(lockedTarget.pos, lockedTarget.velocity, source.x, source.y, source.z)
                    end
                else
                    lookTarget = mouseAim(pInfo.pos, pInfo.lookVector)
                    aimCannon(lookTarget, {x=0, y=0, z=0}, source.x, source.y, source.z)
                end
            end
        else
...
local function hitMark()
    while true do
        ...
        if shipYaw < 0 then shipYaw = shipYaw + 360 end
        local shipPitch = math.deg(ship.getPitch())
        cannonPitch = cannon1.getPitch()
        cannonYaw = cannon1.getYaw()
        if cannonPitch and cannonYaw then
            currentPitch = shipPitch + cannonPitch
            currentYaw = shipYaw + cannonYaw - 90
        end
