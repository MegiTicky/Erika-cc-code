...
local function readConfig(filename)
    if not fs.exists(filename) then
        ...
        EMBTControlChannel = askUser("Input the EMBT control channel", "2000")
        file.writeLine("EMBTControlChannel=" .. EMBTControlChannel)
...
local EMBTControlChannel = tonumber(config.EMBTControlChannel) 
local cannonHitPosChannel = EMBTControlChannel + 10
local reloadChannel = EMBTControlChannel + 20
    ...
    while true do
        if modem then
            ...
            elseif channel == EMBTControlChannel then
                controls = message
            elseif channel == reloadChannel then
...
local function yawControl(deltaYaw, currentYaw, yawRate)
    local now = os.clock()
    local dtThisFrame = 0.05--now - lastYawTime
    lastYawTime = now
    ...
    --print(dtThisFrame)
    local yawSpeed, newIntegral, newPrevError = PIDController(Kp_yaw, Ki_yaw, Kd_yaw, deltaYaw, yawIntegral, (deltaYaw-yawPrevError),yawPrevError, dtThisFrame)
...
lastLockedId, lastYaw, yawRate = nil, nil, nil
lastYawRateTime = os.clock()
local function aimCannon(targetPos, targetVel, sourceX, sourceY, sourceZ, currentPitch, currentYaw)
    if sourceX and sourceY and sourceZ and currentPitch and currentYaw then
        ...
        if lockedTarget and lockedTarget.id then
            estimate = predictFuturePosition(targetPos, targetVel, sourceX, sourceY, sourceZ)
        else
            estimate = targetPos
        end
        local estimateX = estimate.x
        local estimateY = estimate.y
        local estimateZ = estimate.z
        ...
        if lockedTarget and lockedTarget.id then
            if lastLockedId and lastLockedId == lockedTarget.id then
                if currentTime - lastYawRateTime > 0.3 then
                    --have locked previously
                    yawRate = (yaw - lastYaw) / 0.3
                    lastYaw = yaw
                    lastYawRateTime = currentTime
                end
            else
                lastLockedId = lockedTarget.id
                lastYaw = yaw
                yawRate = 0
            end
        else
        ...
        modem.transmit(cannonHitPosChannel,0,{requiredRelativePitch = requiredRelativePitch, requiredRelativeYaw = requiredRelativeYaw}) 

        local deltaYaw = requiredRelativeYaw--(yaw - currentYaw + 180) % 360 - 180
        local deltaPitch = pitch - currentPitch

        -- Turning and pitch adjustment logic
        yawControl(deltaYaw, currentYaw, yawRate)

        print("currentTime: "..currentTime)
        print("lastYawRateTime: "..lastYawRateTime)
        print("yawRate")
        print(yawRate)
        print("deltaYaw: "..deltaYaw)
...
local function setPitchYaw()
    while true do
        if controls and controls.cannonControlMode == "mouseAim" and requiredRelativeYaw and requiredRelativePitch then
            cannon.setPitch(math.max(requiredRelativePitch,-10))
            cannon.setYaw(math.min(math.max(requiredRelativeYaw,-1),1))
        end
        sleep()
    end
end
...
local function targetLockAndAim()
    while true do
        currentPitch,currentYaw = cannon.getPitch(), math.deg(getYaw())
        ...
        if controls.cannonControlMode == "manual" then
            if controls then
                ...
                if controls.cannonUp then
                    -- Increase pitch
                    cannon.setPitch(currentPitch + 1.5)
                    modem.transmit(cannonHitPosChannel,0,{requiredRelativePitch = currentPitch + 1.5}) 
    ...
    while true do
        if controls and controls.fireCannon then
            ...
            if requiredRelativeYaw and requiredRelativePitch then
                cannon.setPitch(math.max(requiredRelativePitch,-10))
                cannon.setYaw(math.min(math.max(requiredRelativeYaw,-6),6))
            end
            cannon.fire()
            sleep(0.1)
            cannon.disassemble()
            ...
            -- Send reload start signal
            modem.transmit(reloadChannel, reloadChannel, {startReload = true})
            redstone.setOutput("back", true)
            reloadMessage_recived.reloaded = false

            -- Wait for reload confirmation
            print("Waiting for reload confirmation...")

            while true do
                if reloadMessage_recived and reloadMessage_recived.reloaded then
                    print("Reload complete! Assembling cannon.")
                    sleep(0.15)
                    cannon.assemble()
                    break
                end
            ...
            modem.transmit(reloadChannel, reloadChannel, {startReload = false})
        end
        sleep()
    end
end
