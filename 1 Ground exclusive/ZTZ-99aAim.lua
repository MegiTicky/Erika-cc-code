    ...
    if math.abs(deltaYaw) > 0.9 and math.abs(deltaYaw) < 1.2 then
        ...
        if controls.accelerate or controls.decelerate then
            if controls.turnLeft then
                --turn level -1
                turnFactor = math.min(turnFactor + 4, 18)
            elseif controls.turnRight then
                --turn level 1
                turnFactor = math.max(turnFactor - 4, -18)
            else
            ...
            if controls.turnLeft then
                --turn level -2
                turnFactor = math.min(turnFactor + 2, 20)
            elseif controls.turnRight then
                --turn level 2
                turnFactor = math.max(turnFactor - 2, -20)
            else
                if turnFactor > 6 then 
                    turnFactor = turnFactor - 6
                 elseif turnFactor < -6 then
