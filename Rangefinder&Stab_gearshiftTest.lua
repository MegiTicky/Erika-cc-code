...
pitchMotor = peripheral.find("Create_SequencedGearshift")
sleep(0.2)
...
local function verticalStabilizerMain()
    while true do
        if enableVerticalStablizer then
            ...
            -- Calculate the delta pitch (difference between current pitch and last pitch)
            local deltaPitch = currentPitch - lastPitch
            ...
            -- Stabilizer desired correction (same as before)
            local desiredCannonSpeed = -deltaPitch  -- Using deltaPitch for the stabilizer output
            local stabilizerOutput = desiredCannonSpeed * 60 / 360 * 8

            -- Combine user input and stabilizer output
            local finalCommand = cannonInput + stabilizerOutput
            finalCommand = math.min(math.max(-finalCommand, -256), 256)

            -- Use the deltaPitch directly to rotate the motor
            -- The modifier can be adjusted based on your requirements, usually 1 is a good default
            if deltaPitch > 0.1 then
                print(deltaPitch)
            end
            if deltaPitch > 0 then
                modifier = 1
            elseif deltaPitch < 0 then
                modifier = -1
            end
            --pitchMotor.rotate(math.abs(deltaPitch), modifier)  -- Rotate by deltaPitch
            pitchMotor.rotate(10, modifier)
        end
