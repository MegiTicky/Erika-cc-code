...
local function projectToScreen(x, y, z, userPos, lookVector)
    ...
    projectedY = math.floor(projectedY) + 40
...
local function drawPitchLadder()
    ...
    for i = -maxPitch, maxPitch, pitchStep do
        local pitchOffset = math.tan(math.rad(i - (math.deg(pitch) + pitchCompensation))) * (screenHeight / 2)
...
local function drawFlightInfo()
    local pitch = getPitch()
    local roll = -getRoll()
    local adjustedPitch = math.deg(pitch + math.rad(pitchCompensation))
