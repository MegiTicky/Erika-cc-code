...
local holoWidth, holoHeight = screenWidth/4,screenHeight/4
holo.Resize(holoWidth,holoHeight)
...
function matscalarmul(M, scalar)
    local result = {}
    for i = 1, #M do
        result[i] = {}
        for j = 1, #M[i] do
...
-- Subtract two matrices (m1 and m2) element-wise.
-- The matrices must have the same dimensions.
function matsub(m1, m2)
    -- Check if both matrices have the same size.
    assert(#m1 == #m2, "Matrices must have the same number of rows")
    assert(#m1[1] == #m2[1], "Matrices must have the same number of columns")
...
function lookVectorToAngle(lookVector)
    local lx, ly, lz = lookVector[1], lookVector[2], lookVector[3]
    
    -- Calculate yaw (rotation around the y-axis)
    local playerYaw = math.deg(math.atan2(lz, lx)) - 90

    -- Ensure yaw is within the range of -180 to 180 degrees
    if playerYaw > 180 then
        playerYaw = playerYaw - 360
    elseif playerYaw < -180 then
        playerYaw = playerYaw + 360
    end
...
local function renderElements(elements, cannonHitPos, pInfo, lockedTarget)
    ...
    for _, element in ipairs(elements) do
        ...
        local holoX = math.floor(element.screenX) -- Scale down for better fit
        local holoY = math.floor(element.screenY) -- Adjust scaling as needed
...
local function drawPitchLadder()
    ...
    local roll = -getRoll()
    local screenWidth,screenHeight = holoWidth, holoHeight
    ...
    while true do
        ...
        parallel.waitForAll(
            function()
                ...
                playerYaw, playerPitch = lookVectorToAngle(pInfo.lookVector)
                --print(playerYaw, playerPitch)

                cameraInfo = {
                    position = {x= pInfo.pos[1],y= pInfo.pos[2],z= pInfo.pos[3]},
                    orientation = {x= playerPitch,y= playerYaw,z= 0},
                    lookVector = pInfo.lookVector,
                    vfov_deg = fov,
                    screen_width = holoWidth,
                    screen_height = holoHeight,
                    near = 0,
                    far = 1,
                ...
                if pInfo.nickname then
                    ...
                    for _, object in ipairs(objectList) do
                        ...
                        local screenX, screenY = projectToScreen(object.pos, cameraInfo)
                        ...
                        if distance > 20 then
                            if screenX and screenY then
                                --print("Adding to elements",screenX, screenY)
                                table.insert(elements, {
                                    ...
                                    renderInfo = true,
                                    renderBox = true--isWithinRenderDistance(screenX, screenY, screenWidth / 2, screenHeight / 2, 4000 / GUIscale, 4800 / GUIscale, 3200 / GUIscale)
                                })
                            end
                        end
                    end
                    ...
                    for i,waypoint in ipairs(waypoints) do
                        local screenX, screenY = projectToScreen(waypoint, cameraInfo)
                ...
                if pInfo then
                    ...
                    --local screenX, screenY = projectToScreen(AimTarget, cameraInfo)
                    if screenX and screenY then
...
missileLaunchOffset = {x=0,y=-5,z=0}
...
local function launchMissile(targetType, targetId, targetPos, missileType)
    ...
    local teleportMessage = string.format(
        "vmod teleport %s %.2f %.2f %.2f (%.2f %.2f 0)",
