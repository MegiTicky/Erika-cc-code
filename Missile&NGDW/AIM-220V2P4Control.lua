...
local function computer2DLOS(targetPos, missilePos, currentPitchDeg, currentRollDeg, currentYawDeg)
    ...
    if math.abs(currentRollDeg) > 90 then
        if currentYawDeg >= 0 and currentYawDeg < 90 then
            currentYawDeg = -currentYawDeg
        elseif currentYawDeg >= 90 and currentYawDeg < 180 then
            currentYawDeg = -currentYawDeg
        elseif currentYawDeg > -180 and currentYawDeg < -90 then
            currentYawDeg = -currentYawDeg
        else
            currentYawDeg = -currentYawDeg
        end
    ...
    -- Calculate the (angle between forward and target direction), as the magintude of the vector
    local AngleRadError = math.acos(dotProduct(forward, targetDir))
    local AngleDegError = AngleRadError * 180 / math.pi

    --imaginary camera in front of missile
    LOSRollShipRollDiff = rollAngleDeg - currentRollDeg
    LOS2DVector = { x = math.sin( math.rad(LOSRollShipRollDiff) ), y = math.cos( math.rad(LOSRollShipRollDiff) )}



    -- Output the computed roll and pitch angles
    return LOS2DVector, AngleDegError, LOSRollShipRollDiff
end

-- Convert a vector to a quaternion with a scalar part of 0
local function vectorToQuaternion(v)
...
local Kp_yaw, Ki_yaw, Kd_yaw = 3, 0, 0.2
local Kp_pitch, Ki_pitch, Kd_pitch = 3, 0, 0.2
...
local function terminal()
    ...
    local LOS2DVector, angleError = computer2DLOS(estimatePos, pos, pitch, roll, yaw)
    if angleError > 90 then angleError = 90 end
    local angleRadError = math.rad(angleError)
    forceVector = {x=LOS2DVector.x * (angleRadError), y=LOS2DVector.y * (angleRadError)}

    -- PID control for yaw
    local yawError = forceVector.x
    ...
    local pitchError = forceVector.y
    ...
    rollOutput = 0--math.min(math.max(rollOutput, -10), 10)
