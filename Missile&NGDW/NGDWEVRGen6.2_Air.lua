...
function degreeDiffToPixelDiff(deg, length, fov)
    -- Calculate pixels per degree for both horizontal and vertical axes
    local px_per_deg = width / fov -- Horizontal
    local px_per_deg_y = height / fov -- Vertical

    -- Convert the degree difference to pixel difference
    local pixelDiff = deg * px_per_deg_x

    return pixelDiff
end

function projectToScreen(x, y, z, userPos, lookVector)
    ...
    -- Convert yaw and pitch difference to pixel difference
    local projectedX = degreeDiffToPixelDiff(yawDiff, holoWidth, fov)
    local projectedY = degreeDiffToPixelDiff(pitchDiff, holoHeight, fov)

    -- Adjust for center of screen (projectedY may need further adjustments)
    local centerX = holoWidth / 2
    local centerY = holoHeight / 2
    projectedX = math.floor(centerX + projectedX)
    projectedY = math.floor(centerY - projectedY) -- Adjust the vertical axis for proper orientation
