...
local Kp_yaw, Ki_yaw, Kd_yaw = 0.5, 0, 0.005
local Kp_pitch, Ki_pitch, Kd_pitch = 0.7, 0, 0.005
local Kp_roll, Ki_roll,Kd_roll = 0.03,0,0
...
local Kp_yaw, Ki_yaw, Kd_yaw = 0.25, 0, 0.001
local Kp_pitch, Ki_pitch, Kd_pitch = 0.25, 0, 0.001
...
local function terminal()
    ...
    if target.type == "ship" then
        for i, object in ipairs(results) do
            if object.id == lockedId then
                ...
                if missileSpeed < 100 then rateOfChase = 100 else rateOfChase = missileSpeed end
    ...
    local requiredRelativeYaw, requiredRelativePitch, targetYaw, targetPitch = computeLocalYawPitch(targetPos, pos)
    ...
    if missileSpeed < 60 then
        ...
        if pitchOutput > 10 or missileSpeed > 150 then
            redstone.setAnalogOutput("top",0)
        end
    end
end
...
local function fuze()
    ...
    if timeToHit < 0.5 and deltaDistance > 0 and distance < 100 then
        sleep(math.max(timeToHit+0.2, 0)) -- Prevent negative sleep time
