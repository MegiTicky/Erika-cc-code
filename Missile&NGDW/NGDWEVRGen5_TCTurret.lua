        ...
        if requiredRelativeYaw and requiredRelativePitch then
            ...
            for index, cannon in pairs(cannons) do
                ...
                local function adjustCannon()
                    ...
                    cannon.setPitch(math.min(math.max(adjustedPitch,-20),70))
                    if controls and controls.cannonControlMode == "mouseAim" then
                        cannon.setYaw(math.min(math.max(adjustedYaw,-2),2))
                    end
                end
...
local function predictFuturePosition(targetPos, targetVel, travelTime)
    ...
    local estimateX = targetPos.x + (targetVel.x * velocityFactor - shipVelocity.x) * estimateTime
    local estimateY = targetPos.y + (targetVel.y * velocityFactor - shipVelocity.y) * estimateTime
    local estimateZ = targetPos.z + (targetVel.z * velocityFactor - shipVelocity.z) * estimateTime
...
local function aimCannon(targetPos, targetVel, sourceX, sourceY, sourceZ, currentPitch, currentYaw)
    if sourceX and sourceY and sourceZ and currentPitch and currentYaw then
        ...
        local futurePos = predictFuturePosition(targetPos, targetVel)
