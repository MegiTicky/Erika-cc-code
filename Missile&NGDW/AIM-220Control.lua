...
jet.setOutputThrust(0)

local controls = {}
local target = {}
local lockedId = nil
local targetPos, targetVel, TargetYaw, TargetPitch
local Kp_yaw, Ki_yaw, Kd_yaw = 0.18, 0.005, 0.008
local Kp_pitch, Ki_pitch, Kd_pitch = 0.24, 0.065, 0.0052
local Kp_roll, Ki_roll,Kd_roll = 0.052,0,0.0035
...
local function initial()
    ...
    if missileSpeed < 340 then
        jet.setOutputThrust(highThrust)  -- Increase throttle
    else
        jet.setOutputThrust(0)
    end
end
...
local function terminal()
    ...
    if target.type == "ship" then
        for i, object in ipairs(results) do
            if object.id == lockedId then
                ...
                local immediateDistance = math.sqrt(dx * dx + dy * dy + dz * dz)
                missileSpeed = math.sqrt(missileVelocity.x^2 + missileVelocity.y^2 + missileVelocity.z^2)
                if missileSpeed < 150 then missileSpeed = 150 end
    ...
    if (deltaDistance < 5 or missileSpeed < 60) and missileSpeed < 270 then
        if (yawOutput < 7 or pitchOutput < 7) and missileSpeed > 80 then
            jet.setOutputThrust(highThrust) --target aimmed, missile speed sufficient, full thrust
        else
            jet.setOutputThrust(0)  --target not aimmed, missile speed sufficient, reduce thrust
        end
    else
        jet.setOutputThrust(0)
        --redrouter.setOutput("front",false)
...
local function fuze()
    ...
    if timeToHit < 0.4 and deltaDistance > 0 and distance < 50 then
        sleep(math.max(timeToHit - 0.15, 0)) -- Prevent negative sleep time
        ...
        for _, output in ipairs(outputs) do
            ...
            redrouter.setOutput(output, false)
        end
