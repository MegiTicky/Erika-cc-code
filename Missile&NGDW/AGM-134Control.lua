...
local function broadCastid()
    while true do
        local id = ship.getId()
        local missileInfo = {id = id, type = "AGM-134", launchState = false}
...
local function broadCastid()
    while true do
        local id = ship.getId()
        local missileInfo = {id = id, type = "AGM-134", launchState = true}
        modem.transmit(missileInfoChannel,0,missileInfo)
        sleep()
    end
end
...
local function initial()
    ...
    if missileSpeed < 80 then
        redstone.setAnalogOutput("top",highThrust)  -- Increase throttle
    else
        redstone.setAnalogOutput("top",0)
    end
end
...
local function terminal()
    ...
    if missileSpeed < 60 then
        ...
        if yawOutput > 10 or pitchOutput > 10 then
            redstone.setAnalogOutput("top",0)
        end
    end
end
...
local function fuze()
    ...
    if timeToHit < 0.4 and deltaDistance > 0 and distance < 20 then
        sleep(math.max(timeToHit - 0.5, 0)) -- Prevent negative sleep time
