    ...
    if target.type == "ship" then
        for i, object in ipairs(results) do
            if object.id == lockedId then
                ...
                -- Update pitch, yaw, and distance
                local estimatePos = targetPos--predictFuturePosition(targetPos,targetVel,pos.x,pos.y,pos.z,rateOfClosure)
...
local function fuze()
    ...
    if timeToHit < 2 and deltaDistance > 0 and distance < 100 then
