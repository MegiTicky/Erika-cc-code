...
local function displayTargetInfo(target)
    ...
    monitor.write("[Port gun]")
    monitor.setCursorPos(1, h-4)
    monitor.write("[Starboard gun]")
    monitor.setCursorPos(1, h-3)
    monitor.write("[Bow gun]")
    monitor.setCursorPos(1, h-2)
    monitor.write("[Stern gun]")

    monitor.setCursorPos(20, h-5)
    monitor.write("[AIM-220, Avaialbe: "..AIMCount.."]")
    monitor.setCursorPos(20, h-4)
    monitor.write("[GBU-42, Avaialbe: "..GBUCount.."]")
    monitor.setCursorPos(20, h-3)
    monitor.write("[thunderbolt, Avaialbe: "..thunderboltCount.."]")
...
local function handleTouch()
    ...
    while true do
        ...
        elseif displayState == "targetInfo" then
            ...
            elseif y == h-4 and x >= 1 and x <= 15 then
                ...
                monitor.setCursorPos(1, 1)
                monitor.write("Target locked: " .. (selectedTarget.type == "waypoint" and "Waypoint" or selectedTarget.id))
            elseif y == h-3 and x >= 1 and x <= 9 then
                ...
                monitor.setCursorPos(1, 1)
                monitor.write("Target locked: " .. (selectedTarget.type == "waypoint" and "Waypoint" or selectedTarget.id))
            elseif y == h-2 and x >= 1 and x <= 11 then
                ...
                monitor.write("Target locked: " .. (selectedTarget.type == "waypoint" and "Waypoint" or selectedTarget.id))
            -- AIM-220
            elseif y == h-5 and x >= 20 and x <=42 then
                table.insert(pendingMissileLaunches, {type = "AIM-220", target = selectedTarget})
                displayState = "radar"
                monitor.clear()
                drawRadarDisplay()
                monitor.setCursorPos(1, 1)
                monitor.write("Missile queued: AIM-220")

            -- GBU-42
            elseif y == h-4 and x >= 20 and x <=41 then
                table.insert(pendingMissileLaunches, {type = "GBU-42", target = selectedTarget})
                displayState = "radar"
                monitor.clear()
                drawRadarDisplay()
                monitor.setCursorPos(1, 1)
                monitor.write("Missile queued: GBU-42")

            -- thunderBolt
            elseif y == h-3 and x >= 20 and x <=46 then
                table.insert(pendingMissileLaunches, {type = "thunderBolt", target = selectedTarget})
                displayState = "radar"
                monitor.clear()
                drawRadarDisplay()
                monitor.setCursorPos(1, 1)
                monitor.write("Missile queued: thunderBolt")
            end
