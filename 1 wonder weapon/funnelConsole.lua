...
local RS = {send = "top", recall = "bottom", fire = "front", close = "back"}
...
local funnelMessage = { type = "recall", id = nil, pos = nil, weapon = nil, fire = nil}
local closeCooldown = true
local function funnelHandler()
    while true do
        local weaponChoosen = "funnel"
        
        if redstone.getInput(RS.send) then
            if lockedTarget and lockedTarget.id and missileCoolDown == 0 and type(lockedTarget.id) == "number" then
                funnelMessage.type = "enemy"
                funnelMessage.id   = lockedTarget.id
                funnelMessage.pos  = lockedTarget.pos
                funnelMessage.weapon = weaponChoosen
                missileCoolDown = 20
                print("Sending funnel")
            end
        end

        if redstone.getInput(RS.recall) then
            funnelMessage.type = "recall"
            funnelMessage.id   = ship.getId()
            funnelMessage.pos  = nil
            funnelMessage.weapon = weaponChoosen
            print("Recalling funnel")
        end

        if funnelMessage.type == "recall" then
            local mouseAimTarget
            if pInfo.nickname then
                mouseAimTarget = mouseAim(pInfo.pos, pInfo.lookVector)
            end
            funnelMessage.pos = mouseAimTarget
        end

        if redstone.getInput(RS.close) then
            if closeCooldown then
                if funnelMessage.type == "closeInitiate" then
                    funnelMessage.type = "closeExecute"
                else
                    funnelMessage.type = "closeInitiate"
                end
                closeCooldown = false
            end
        else
        ...
        if redstone.getInput(RS.fire) then
            funnelMessage.fire = true
        else
            funnelMessage.fire = false
        end



        modem.transmit(missileControlChannel, missileControlChannel, funnelMessage)
...
local function printOutput()
    while true do
        ...
        print(textutils.serialize(funnelMessage))
        sleep(0.5)
    end
end
