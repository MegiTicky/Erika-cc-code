...
local function createPresetSettings()
    local presets = {
        ...
        {
            filename = "TuoCChiangCIWS_settings.txt",
            settings = {
                controlsChannel = 500,
                velocityFactor = 1,
                projectileSpeed = 120,
...
local function predictFuturePosition(targetPos, targetVel, sourceX, sourceY, sourceZ)
    ...
    local estimateTime = distance / rateOfClosure + 0.5
...
local function aimCannon(targetPos, targetVel, targetAcc, sourceX, sourceY, sourceZ)
    if sourceX and sourceY and sourceZ then
        ...
        print(projectileSpeed,g,cd,sourceX,pitch)
        ...
        requiredRelativeYaw = yaw - shipYaw
...
local function setPitchYaw()
    while true do
        if requiredRelativeYaw and requiredRelativePitch then
            --print("setting angle yaw: "..requiredRelativeYaw.." pitch: "..requiredRelativePitch)
