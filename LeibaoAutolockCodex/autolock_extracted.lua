...
local yawLosFeedForwardBlend = 0.75
...
local function estimateHorizontalYawRate(source, aimPos, relativeVelocity)
    local dx, dz = aimPos.x - source.x, aimPos.z - source.z
...
local function clearTracking(reason)
    ...
    smoothedTargetYawRate, smoothedLosYawRate, smoothedYawOutput, smoothedYawDerivative = 0, 0, 0, 0
    stopYawMotor()
...
local function trackTarget(target, modeName)
    ...
    local losYawRate = estimateHorizontalYawRate(source, futurePos, relativeVelocity)
    setPitchYaw(relPitch, localCannonYaw)
    rotateTurret(relYaw, worldYaw, losYawRate)
