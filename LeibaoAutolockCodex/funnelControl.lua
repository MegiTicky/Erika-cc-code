cannon = peripheral.find("cbc_cannon_mount")
...
RS = {fire = "right", sticker = "back"}
redstone.setOutput(RS.sticker, true)
sleep(0.2)
redstone.setOutput(RS.sticker, false)
muzzleVelocity = 60
...
Kp_torque, Ki_torque, Kd_torque = 650, 0, 50
function aimShip(targetInfo)
    ...
    yawOutput = -math.max(math.min(yawOutput,100000),-100000)
    ...
    pitchOutput = -math.max(math.min(pitchOutput,100000),-100000)
...
local tangentialForceMag = 100000 * math.random(1,1.4)  -- how hard you "push sideways" to keep orbiting

local Kp_radial, Kd_radial = 10000, 2000
local Kp_vert,   Kd_vert   = 10000, 2000
...
function handleFiring()
    while true do
        if lockedTarget and lockedTarget.fire then
            local fireCount = 0
            cannon.fire()
            cannon.fire()
            cannon.fire()
