...
local startX = 50
local startY = 50
local sky_color = "1" -- Blue
local ground_color = "6" -- Brown
...
local function hexColorFromCode(code)
    ...
    return colors[code] or "000000" -- Default to black if the code is not found
end
...
local function get_pixel_depthmap(item)
    if item.distance == nil or item.distance > max_distance then 
        -- Determine if the ray hit the sky or the ground
        local is_sky = item.hit and item.hit == "sky" -- Adjust this condition based on your raycaster's API
        local color = is_sky and sky_color or ground_color
        return { " ", "b", "b" }
    end
...
local function renderLine()
    ...
    if ship then
        local rotationalMatrix = ship.getRotationMatrix()
        pitch = toDegrees(ship.getRoll())
        roll = toDegrees(ship.getPitch())
        print(roll)
    end
    local width, height = monitor.getSize()
    local text = "----"..math.floor(pitch).."----"
    local fgColor = string.rep("0", #text)
    local bgColor = string.rep("b", #text)

    monitor.setCursorPos(math.floor(width / 2 - #text / 2 + 1), math.floor(height / 2))
    monitor.blit(text, fgColor, bgColor)

    local startX = math.floor(width / 2) + 1  -- Centered horizontally
    local startY = math.floor(height / 2) - 4 -- Adjust as needed for vertical center

    -- Draw the vertical line
    for y = startY, startY + 3 do  -- Draw a line of height 20 blocks
        monitor.setCursorPos(startX, y)
        monitor.blit("|", "0", "b")  -- "|" character, white foreground, black background
    end
    local textRoll = tostring(math.abs(round(roll)))
    local rollLength = #textRoll
    local rollStartX = startX - math.floor(rollLength / 2)  -- Calculate the x-coordinate to center the roll data

    -- Write the roll data centered above the vertical line
    monitor.setCursorPos(rollStartX, startY)
    monitor.setBackgroundColor(0x800)
    monitor.write(textRoll)
end

local function raycast_and_render(x_start, x_end, y_start, y_end)
    for y = y_start, y_end do
        ...
        for x = x_start, x_end do
            ...
            local item
            if euler_mode then
                item = rc.raycast(max_distance, { yr, xr }, euler_mode, true)
            else
                item = rc.raycast(max_distance, { yr, xr, vector_fov }, euler_mode, true)
            end
...
while true do
    start = os.epoch("utc")
    parallel.waitForAll(
        function() raycast_and_render(1, math.floor(width/2), 1, math.floor(height/2)) end,
        function() raycast_and_render(math.floor(width/2) + 1, width, 1, math.floor(height/2)) end,
        function() raycast_and_render(1, math.floor(width/2), math.floor(height/2) + 1, height) end,
        function() raycast_and_render(math.floor(width/2) + 1, width, math.floor(height/2) + 1, height) end
    )
