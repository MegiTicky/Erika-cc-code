...
-----------------
--5inch
local MAG_SIZE    = 60       -- rounds per magazine
local RELOAD_TIME = 3       -- seconds
local FIRE_RPM    = 300       -- rounds per minute
FIRE_RPM = FIRE_RPM * 2
local elevateTraverseSpeed = 2 -- angle per 0.5s
local maxYawSpeed = 8
-- spread settings (AREA at convergence plane, in blocks^2)
local spreadArea = 40
-- basllist
local projectileSpeed = 320  -- blocks/sec (set to your gun's muzzle velocity)
local g = 0.05
local cd = 0.995
...
AntiAirChannel = askUser("Input the channel number",960)
...
local Kp_yaw, Ki_yaw, Kd_yaw = 0.1, 0.04, 0.04
...
local function yawControl(deltaYaw)
    ...
    yawSpeed = math.max(-maxYawSpeed, math.min(maxYawSpeed, yawSpeed))
...
originalYaw = cannons[1].getYaw()


local function moveTurret(yaw, pitch, firing, convDist)
    firing = (firing == true)
    local tasks = {}  -- Table to store tasks for parallel execution
    --print("detla yaw: "..yaw)

    local deltaYaw = clampAngle180(yaw + originalYaw)
    yawControl(deltaYaw)
...
local function fireCannonsWithSpread(trigger)
    ...
    if trigger then
        ...
        local currentYaw, currentPitch = cannons[1].getYaw(), cannons[1].getPitch()

        local tasks = {}  -- Table to store tasks for parallel execution
        for i, cannon in ipairs(cannons) do
            local task = function()
                cannon.setYaw(currentYaw + cannonSpread[i].yawOff)
                cannon.setPitch(currentPitch + cannonSpread[i].pitchOff)
            end
        ...
        for _, cannon in ipairs(cannons) do
            cannon.setYaw(currentYaw)
            cannon.setPitch(currentPitch)
        end
    end
end
    ...
    function()
        while true do
            ...
            elseif recievedInfo and recievedInfo.target and recievedInfo.target.x and not recievedInfo.pausing then
                ...
                moveTurret(requiredRelativeYaw, requiredRelativePitch, recievedInfo.isFiring, recievedInfo.convergenceDist)

                if safe then
                    fireCannonsWithSpread(recievedInfo.isFiring)
                end
                reseted = false
            else
                yawMotor.setTargetSpeed(0)
