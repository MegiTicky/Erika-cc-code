...
local frontRouter = peripheral.find("redrouter")
...
print("Do you need front and back router, 1 for yes,0 for no")
local yesOrNo = io.read()
local includeBowAndStern = false
if yesOrNo == "1" then
    includeBowAndStern = true
    print("connect Front router for bow turret, disconnect back router, and press enter")
    io.read()
    frontRouter = peripheral.find("redrouter")

    print("Connect back router and disconnect front router, then press enter")
    io.read()
    backRouter = peripheral.find("redrouter")

    print("Connect front and back router, then press enter")
    io.read()
end
...
local function calculateRange(angle, u, cd, g, c_est, vm)
...
local function findBestPitch(targetX, targetY, targetZ, sourceX, sourceY, sourceZ, initialVelocity, g, cd, c_est, vm)
    ...
    for pitch = 0, 70, 0.01 do -- Iterate over pitch angles
        local calculatedRange = calculateRange(pitch, initialVelocity, cd, g, c_est, vm)
...
local function aimCannon(targetPos, targetVel, sourceX, sourceY, sourceZ, currentPitch, currentYaw, side)
    if sourceX and sourceY and sourceZ and currentPitch and currentYaw then
        -- Set the gravitational constant and muzzle velocity based on the turret type
        local g, vm
        if side == "port" or side == "starboard" then
            g = 0.04
            vm = 330
        elseif side == "bow" or side == "stern" then
            g = 0.05
            vm = 280
        end
        ...
        local estimateTime = distance / vm / 2
        ...
        -- Use the custom formula to find the best pitch
        local pitch = findBestPitch(estimateX, estimateY, estimateZ, sourceX, sourceY, sourceZ, vm, g, 0.99, 0.0028, vm)
        local yaw = math.deg(math.atan2(-dx, dz)) + 180
        print(pitch)
        if pitch < 30 then
            pitch = pitch + math.deg(math.atan2(dy, horizontalDistance))
        end
        local adjustedPitch, adjustedYaw
        -- Apply pitch and yaw adjustments specific to the cannon
        if side == "port" or side == "starboard" then
            adjustedPitch = pitch + cannonTarget[side.."5inch"].pitchAdjust
            adjustedYaw = yaw + cannonTarget[side.."5inch"].yawAdjust
        elseif side == "bow" or side == "stern" then
            adjustedPitch = pitch + cannonTarget[side.."15inch"].pitchAdjust
            adjustedYaw = yaw + cannonTarget[side.."15inch"].yawAdjust
        end
        print(side.." adjustedPitch: "..adjustedPitch)
        -- Calculate angle differences
        local deltaYaw = (adjustedYaw - currentYaw + 180) % 360 - 180
        local deltaPitch = adjustedPitch - currentPitch

        -- Determine which router to use based on side
        local yawRouter, pitchRouter
        if side == "port" then
            yawRouter = leftRouter
            pitchRouter = leftRouter -- Adjust this if pitch is controlled by a different mechanism
        elseif side == "starboard" then
            yawRouter = rightRouter
            pitchRouter = rightRouter -- Adjust this if pitch is controlled by a different mechanism
        elseif side == "bow" then
            yawRouter = frontRouter
            pitchRouter = frontRouter
        elseif side == "stern" then
            yawRouter = backRouter
            pitchRouter = backRouter
        end

        local yaw_tolerance = 0.3
        local pitch_tolerance = 0.08
        --yaw adjustment
        if math.abs(deltaYaw) > yaw_tolerance then
            if deltaYaw < 0 then
                print("Turning left")
                yawRouter.setOutput("right", false)
                if math.abs(deltaYaw) < 10 then
                    local turnTime = math.max(math.abs(deltaYaw) / 6,0.1)
                    yawRouter.setOutput("left", true)
                    sleep(turnTime)
                    yawRouter.setOutput("left", false)
                else
                    yawRouter.setOutput("left", true)
                end
            elseif deltaYaw > 0 then
                ...
                if math.abs(deltaYaw) < 10 then
                    local turnTime = math.max(math.abs(deltaYaw) / 6,0.1)
                    yawRouter.setOutput("right", true)
                    sleep(turnTime)
                    yawRouter.setOutput("right", false)
                else
                    yawRouter.setOutput("right", true)
        ...
        if math.abs(deltaPitch) > pitch_tolerance then
            if deltaPitch > 0 then
                ...
                pitchRouter.setOutput("front", false)
                if math.abs(deltaPitch) < 5 then
                    local turnTime = math.max(math.abs(deltaPitch) / 3,0.1)
                    pitchRouter.setOutput("top", true)
                    sleep(turnTime)
                    pitchRouter.setOutput("top", false)
                else
                    pitchRouter.setOutput("top", true)
                end
            else
                ...
                if math.abs(deltaPitch) < 5 then
                    local turnTime = math.max(math.abs(deltaPitch) / 3,0.1)
                    pitchRouter.setOutput("front", true)
                    sleep(turnTime)
                    pitchRouter.setOutput("front", false)
                else
                    pitchRouter.setOutput("front", true)
    ...
    while true do
        ...
        if cannon.type then
            ...
            if target and target.pos then
                aimCannon(target.pos, target.velocity, cannonData.port5inch.source.x, cannonData.port5inch.source.y, cannonData.port5inch.source.z, cannonData.port5inch.pitch, cannonData.port5inch.yaw, "port", 330)
            end
        else
            if leftRouter.setOutput then
                -- Ensure that outputs are reset when not actively aiming
                leftRouter.setOutput("top", false)
                leftRouter.setOutput("left", false)
                leftRouter.setOutput("right", false)
...
local function aimStarboardCannonContinuous()
    while true do
        ...
        if cannon.type then
            ...
            if rightRouter.setOutput then
                -- Ensure that outputs are reset when not actively aiming
                rightRouter.setOutput("top", false)
                rightRouter.setOutput("left", false)
                rightRouter.setOutput("right", false)
    ...
    while true do
        ...
        if frontRouter then
            if cannon.type then
                ...
                if frontRouter and frontRouter.setOutput then
                    -- Ensure that outputs are reset when not actively aiming
    ...
    while true do
        ...
        if backRouter then
            if cannon.type then
                ...
                if backRouter and backRouter.setOutput then
                    -- Ensure that outputs are reset when not actively aiming
