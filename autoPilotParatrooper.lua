...
-- =========================
-- Allocation-free ship angle helpers
-- Replace normalizeVector(), normalizeRotationMatrix(),
-- getPitch(), getYaw(), getRoll(), and the buildDirection wrapper section
-- =========================

local buildDirection = "+x"
local yawOffsetDeg = 0

local function safeNormalize3(x, y, z)
...
local function getPitch()
    ...
    -- old code normalized row 2 and then used [2][3]
    local _, _, r23 = safeNormalize3(m[2][1], m[2][2], m[2][3])

    return math.asin(r23)
end
-- Get the yaw of the ship
local function getYaw()
    local m = ship.getTransformationMatrix()

    -- old code normalized row 3 and then used atan2(-[3][1], -[3][3])
    local r31, _, r33 = safeNormalize3(m[3][1], m[3][2], m[3][3])

    return math.atan2(-r31, -r33)
end
-- Get the roll of the ship
local function getRoll()
    local m = ship.getTransformationMatrix()

    -- old code normalized row 2 and then used atan2([2][1], [2][2])
    local r21, r22, _ = safeNormalize3(m[2][1], m[2][2], m[2][3])
...
local function autopilot(targetCoord)
    ...
    while true do
        ...
        if dist > radius then
            ...
            local targetPositionX = pos.x + 200 * math.cos(math.rad(targetYaw))
            local targetPositionZ = pos.z + 200 * math.sin(math.rad(targetYaw))

            -- These are not actually used later, so you can delete them entirely.
            --local dxMove = targetPositionX - pos.x
            --local dzMove = targetPositionZ - pos.z
            ...
            -- Debug
            print(string.format("[LOITER] Outside target area. Moving straight to position %.1f, %.1f", targetPositionX, targetPositionZ))
