...
local width_yaw_range = math.rad(30)
local height_pitch_range = math.rad(30)
...
local function get_pixel_depthmap(item)
    ...
    return {tostring(x),tostring(y)," ","c","c"}
end
...
local function handleControl()
    while true do
        ...
        if channel == controlChannel then
            vector_fov = vector_fov + message
        end
        print(vector_fov)
    end
end

local function raycast_and_render(x_start, x_end, y_start, y_end, channel)
    for y = y_start, y_end do
        for x = x_start, x_end do
            ...
            modem.transmit(imageChannel,0,pix)
            monitor.blit(pix[3],pix[4],pix[5])
            
        end
    end
end

local function raycast_Distance()

    local object = rc.raycast(max_distance,{0,0,1},euler_mode,true)
    modem.transmit(distanceChannel,0,object.distance)
...
parallel.waitForAny(
    function()
        while true do
            start = os.epoch("utc")
            parallel.waitForAll(
                function() raycast_and_render(1, math.floor(width/2), 1, math.floor(height/2), imageChannel1) end,
                function() raycast_and_render(math.floor(width/2) + 1, width, 1, math.floor(height/2), imageChannel2) end,
                function() raycast_and_render(1, math.floor(width/2), math.floor(height/2) + 1, height, imageChannel3) end,
                function() raycast_and_render(math.floor(width/2) + 1, width, math.floor(height/2) + 1, height, image4) end,
                function() raycast_Distance() end
