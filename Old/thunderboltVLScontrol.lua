...
local function initial()
    ...
    if missileSpeed < 60 then
        modem.transmit(throttleChannel, 0, 28 * multiplier)  -- Increase throttle
    else
        modem.transmit(throttleChannel, 0, -4 * multiplier)
    end
end
...
local function fuze()
    ...
    --impact fuze
    if redstone.getInput("left") and distance < 70 then
        print("detonating")
        redstone.setOutput("right",true)
        sleep(0.1)
        redstone.setOutput("front",false)
