...
print(peripheral.getType(cannons[1]))
...
local function aimCannon(targetPos, targetVel, sourceX, sourceY, sourceZ)
    if sourceX and sourceY and sourceZ then
        ...
        pitch = pitch + findBestPitch(estimateX, estimateY, estimateZ, sourceX, sourceY, sourceZ, projectileSpeed, 0.025, 0.99, 0.0028, projectileSpeed)
        ...
        if math.abs(math.abs(requiredRelativePitch) - math.abs(cannons[1].getPitch())) > 0 then
            cannons[1].setPitch(requiredRelativePitch)
            cannons[2].setPitch(requiredRelativePitch)
            cannons[3].setPitch(requiredRelativePitch)
            cannons[4].setPitch(requiredRelativePitch)
        end
        if math.abs(requiredRelativeYaw - cannons[1].getYaw()) > 0 then
            cannons[1].setYaw(requiredRelativeYaw)
            cannons[2].setYaw(requiredRelativeYaw)
            cannons[3].setYaw(requiredRelativeYaw)
            cannons[4].setYaw(requiredRelativeYaw)
        end
    end
end
        ...
        if controls.cannonControlMode == "manual" then
            if controls then
                ...
                if controls.cannonUp then
                    ...
                    cannons[3].setPitch(pitch + 2)
                    cannons[4].setPitch(pitch + 2)
                ...
                if controls.cannonLeft then
                    -- Turn left
                    cannons[1].setYaw(yaw - 2)
                    cannons[2].setYaw(yaw - 2)
                    cannons[3].setYaw(yaw - 2)
                    cannons[4].setYaw(yaw - 2)
