...
local projectileSpeed1 = 340
local projectileSpeed2 = 160
...
print("Press enter to assemble cannon1 (medium cannon)")
io.read()

if cannon1.assemble() then
    print("asseble failed, press enter to contiue")
    io.read()
end

print("Now build cannon2 (big cannon at the front)")
if cannon2.assemble() then
    print("asseble failed, press enter to contiue")
    io.read()
end
...
local function aimCannon1(targetPos, targetVel, sourceX, sourceY, sourceZ)
    if sourceX and sourceY and sourceZ then
        ...
        local estimateTime = distance / (330)  -- assuming velocity units and time units need adjustment
        ...
        pitch = pitch + findBestPitch(estimateX, estimateY, estimateZ, sourceX, sourceY, sourceZ, projectileSpeed1, 0.04, 0.99, 0.0028, projectileSpeed1)
        ...
        if math.abs(math.abs(requiredRelativePitch) - math.abs(cannon1.getPitch())) > 0 then
            cannon1.setPitch(requiredRelativePitch)
        end
        if math.abs(requiredRelativeYaw - cannon1.getYaw()) > 0 then
            cannon1.setYaw(requiredRelativeYaw)
        end
    end
end

local function aimCannon2(targetPos, targetVel, sourceX, sourceY, sourceZ)
    if sourceX and sourceY and sourceZ then
        local dx = targetPos.x - sourceX
        local dy = targetPos.y - sourceY
        local dz = targetPos.z - sourceZ
        ...
        pitch = pitch + findBestPitch(estimateX, estimateY, estimateZ, sourceX, sourceY, sourceZ, projectileSpeed2, 0.05, 0.99, 0.0028, projectileSpeed2)
        ...
        local requiredRelativeYaw,requiredRelativePitch = findRelativeAngle(yaw,pitch)
        if math.abs(math.abs(requiredRelativePitch) - math.abs(cannon1.getPitch())) > 0 then
            cannon2.setPitch(requiredRelativePitch)
        end
        if math.abs(requiredRelativeYaw - cannon1.getYaw()) > 0 then
            cannon2.setYaw(requiredRelativeYaw)
        end
    end
end
            ...
            if pInfo.nickname then
                ...
                if lockedTarget then
                    ...
                    if lockedTarget.pos then
                        parallel.waitForAll(
                            function() aimCannon1(lockedTarget.pos, lockedTarget.velocity, source.x, source.y, source.z) end,
                            function() aimCannon2(lockedTarget.pos, lockedTarget.velocity, source.x, source.y, source.z) end
                        )
                    end
                else
                    lookTarget = mouseAim(pInfo.pos, pInfo.lookVector)
                    parallel.waitForAll(
                        function () aimCannon1(lookTarget, {x=0, y=0, z=0}, source.x, source.y, source.z) end,
                        function () aimCannon2(lookTarget, {x=0, y=0, z=0}, source.x, source.y, source.z) end
                    )              
                end
            end
        else
...
local function manualCannonControl()
    while true do
        if controls.cannonControlMode == "manual" then
            if controls then
                local pitch = cannon1.getPitch()
                local yaw = cannon1.getYaw()
                -- Control yaw and pitch manually with arrow keys
                if controls.cannonUp then
                    -- Increase pitch
                    cannon1.setPitch(pitch + 2)
                    cannon2.setPitch(pitch + 2)
                ...
                if controls.cannonLeft then
                    -- Turn left
                    cannon1.setYaw(yaw - 2)
                    cannon2.setYaw(yaw - 2)
...
local function hitMark()
    while true do
        ...
        local currentPitch = shipPitch + cannon1.getPitch()
        local currentYaw = shipYaw + cannon1.getYaw() - 90
        if currentYaw < 0 then currentYaw = currentYaw + 360 end
        source = ship.getWorldspacePosition()
        print("curretYaw: "..currentYaw)
        if source and currentYaw and currentPitch then
            ...
            objY = source.y+2 + dy
            objZ = source.z + dz
            print("objX: "..objX.." objY: "..objY.." objZ: "..objZ)
        end
