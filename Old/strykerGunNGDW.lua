...
local function mouseAim(userPos, lookVector)
    ...
    local lx, ly, lz = lookVector[1], lookVector[2], lookVector[3]
    local playerYaw = math.deg(math.atan2(lz, lx))
    local playerPitch = math.deg(math.asin(-ly)) - 15
    playerYaw = playerYaw + 90
    if playerYaw < 0 then
        playerYaw = playerYaw + 360
    end
...
local function setMotorSpeed(motor, speed, motorType)
    ...
    local updateTolerance = 1

    if currentTime - lastUpdateTime >= updateInterval then
        if math.abs(motor.getSpeed() - speed) > updateTolerance then
...
local function aimCannon(targetPos, targetVel, sourceX, sourceY, sourceZ, currentPitch, currentYaw)
    if sourceX and sourceY and sourceZ and currentPitch and currentYaw then
        ...
        local estimateTime = distance / (6 * 20)  -- assuming velocity units and time units need adjustment
        ...
        local pitchAdjustments = {
            {min = 50, max = 100, increment = 1},
            {min = 180, max = 250, increment = 2},
            {min = 250, max = 300, increment = 3},
            {min = 300, max = 350, increment = 4},
            {min = 350, max = 400, increment = 6},
        ...
        for _, adjustment in ipairs(pitchAdjustments) do
            if distance > adjustment.min and distance <= adjustment.max then
        ...
        local pidYaw = Kp * yawError
        yawErrorSum = yawErrorSum + yawError * dt
        if yawError < 0 then pidYaw = -pidYaw end
        local yawRateMismatch = math.abs(yawDerivative) - math.abs(pidYaw * 6 *dt)
        print("pidYaw: "..pidYaw)
        print("pidYaw * 360/60*dt: "..pidYaw * 360/60*dt)
        print("yawDerivative: "..yawDerivative)
        print("yawRateMismatch: "..yawRateMismatch)
        local pidYaw = pidYaw
        ...
        local pitchSpeedScale = -math.max(math.abs(deltaPitch)/4, minSpeed) * 8

        -- Turning and pitch adjustment logic
        if math.abs(yawError) > 1 then
            setMotorSpeed(yawMotor, yawError < 0 and pidYaw or -pidYaw, "yaw")
        else
            setMotorSpeed(yawMotor, 0, "yaw")
        end

        if math.abs(deltaPitch) > 1 then
